...
The same one that shaped [`docs/adr-run-ownership.md`](adr-run-ownership.md): `RunInfo` deliberately
...
`owner_secret` residual in [`docs/adr-run-ownership.md`](adr-run-ownership.md) and is left to client
...
Unchanged from [`docs/adr-run-ownership.md`](adr-run-ownership.md): no address is stored or compared
