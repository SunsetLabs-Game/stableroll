...
| Component | Status |
|---|---|
| `contracts/payroll` — the `Payroll` Cairo contract, `privacy_invoke`-driven run accounting, run-ownership authorization | Implemented, tested (`snforge test`, CI-enforced) |
| Starknet-to-Starknet fund + claim, via the pool | Implemented, tested against a mocked/local setup. Not yet exercised against live Sepolia or mainnet infrastructure — see the open issues on live Sepolia claim tests and mainnet eligibility. |
| EVM claim leg (privacy-bridge) | Not implemented yet |
| Solana claim leg (NEAR Intents) | Not implemented yet |
| Waku recipient notification | Not implemented yet |
| Cavos payer/recipient UX | Not implemented yet |
...
### TypeScript — the tokenless path (no credentials needed)
...
If you hit a build/toolchain error, check `CLAUDE.md` §3 — its
